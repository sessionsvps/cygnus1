...
async def logUserBot():
    load_dotenv()
    api_id = int(25824668)
    api_hash = "df5daa30639f0672eba2927839e352c9"
    phone_number = "51960434465"
    ...
    while True:
        ...
        try:
            await client.send_message("@SpamCygnus", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)-1}</code>", parse_mode="HTML")
        except:
            pass
                    ...
                    if i['group_id'] in special_group_ids:  # Compara por group_id en lugar de group_name
                        ...
                        for message_spam in special_messages_list:
                            ...
                            try:
                                ...
                                await client.send_message("@SpamCygnus", f'<b>Error enviando mensajes a {i["group_id"]}</b> - <code>{i["group_name"]}<code>\nCausa:{error}', parse_mode="HTML")
                                resultado = False
                            if resultado:
                                await client.send_message("@SpamCygnus", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>', parse_mode="HTML")  
                        ...
                        for message_spam in messages_list:
                            ...
                            if j == 3: break
            await client.send_message("@SpamCygnus", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
